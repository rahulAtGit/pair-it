 ...
 */
var polyfilfn = function(exports) {
  ...
  PropertyValueParser.prototype = {
    ...
    parseParam: function(params) {
      ...
      if (spaceStart !== -1) {
        ...
        if (commaStart !== -1) {
          unit = params.substr(spaceStart + 1, commaStart - (spaceStart +
            1)).trim();
        } else {
    ...
    parse: function(property, value) {
      ...
      if (value.indexOf('(') !== -1 && value.split("(").length === value.split(
          ")").length) {
        ...
        while ((i = value.indexOf("attr(", last)) !== -1) {
          ...
          var param = this.parseParam(value.substr(funcStart, funcEnd -
            funcStart));
          if (param[0] === '') return null; // Empty attr() function

          // Translate it to Javascript
          result.push('(n.hasAttribute("' + param[0] +
            '") ? (n.getAttribute("' + param[0] + '") + "' + param[1] +
            '") : "' + param[2] + '" + "' + param[1] + '")');
      ...
      parse: function(css) {
        ...
        while ((ruleMatch = cssRule.exec(css)) !== null) {
          ...
          while ((propMatch = cssParse.exec(body)) !== null) {
            var parsedCssValue = propertyValueParser.parse(propMatch[
              1], propMatch[2]);
            // Ignore 'Content' css property.
            if (parsedCssValue !== null && parsedCssValue.cssProp !==
              'content') {
  ...
  NodeObserver.prototype = {
      ...
      var observer = new this.mutationObserver(function(mutationList) {
        ...
        for (var i = 0; i < mutationList.length; i++) {
          ...
          if (node.getAttribute(attrName) !== mutation.oldValue) {
            styleGenerator.resolveFuncList(node, styles, attr[
              attrName]);
          }
        }
    ...
    attach: function(ast) {
      for (var selector in ast) {
        ...
        for (var i = 0; i < nodeList.length; i++) {
          var node = nodeList[i];
          this.observerList.push(this.add(node, selectorData.attr,
            selectorData.filter));
        }
      }
    }
